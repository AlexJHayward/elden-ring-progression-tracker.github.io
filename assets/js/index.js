...
async function calculate() {
  ...
  Object.keys(itemsData).forEach((region) => {
    ...
    markdownContent += `## ${region}\n\n`;

    Object.keys(itemsData[region]).forEach((zone) => {
      let itemsToInsert = "";
      let counter = 0;

      markdownContent += `### ${zone}\n\n`;
      ...
      Object.keys(itemsData[region][zone]).forEach((itemKey) => {
        if (id_list.includes(itemKey)) {
          ...
          const fextraLink = `[FextraLife](https://eldenring.wiki.fextralife.com/${sanitizeURL(itemsData[region][zone][itemKey].name)})`;
          const sanitisedHint = itemsData[region][zone][itemKey].hint
            .replaceAll("&nbsp;", " ")
            .replaceAll(hintTagRegex, "");
          markdownContent += `- [ ] **${itemsData[region][zone][itemKey].name}** (${itemsData[region][zone][itemKey].type}) ${fextraLink} ${sanitisedHint}\n`;
        }
      });
      //Quantifiable icons
  ...
  //Global completion
  const completion = `<h2>Completion: ${Math.floor((globalCounter / globalTotal) * 100)}%</h2> <button onclick="copyMarkdownToClipboard()">Copy text</button>`;
